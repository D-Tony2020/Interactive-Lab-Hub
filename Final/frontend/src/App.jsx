...
const Sidebar = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <div className="w-24 bg-slate-900 border-r border-slate-800 flex flex-col items-center py-6 space-y-8 h-full z-20">
      <div className="w-12 h-12 rounded-xl bg-blue-600 flex items-center justify-center shadow-lg shadow-blue-900/50 mb-4">
        <div className="text-white font-bold text-xl">OS</div>
      </div>
      ...
      <button className="p-4 rounded-full bg-slate-800 text-blue-400 hover:bg-blue-900/30 transition-colors relative group">
        <Mic size={24} />
      </button>
    </div>
  );
...
const RecipeView = ({ inventory }) => {
  // Static recipes for demo, but matched against dynamic inventory
  const recipes = [
    { id: 1, name: 'Salmon Salad', ingredients: ['Salmon', 'Spinach', 'Lemon'], time: '15 min', type: 'seafood' },
    { id: 2, name: 'Omelette', ingredients: ['Egg', 'Milk', 'Cheese'], time: '10 min', type: 'egg' },
    { id: 3, name: 'Fruit Bowl', ingredients: ['Apple', 'Banana', 'Yogurt'], time: '5 min', type: 'fruit' },
  ];

  const getMatchStatus = (ingredients) => {
    ...
    return { available, missing, percentage: Math.round((available.length / ingredients.length) * 100) };
  };

  return (
    <div className="p-6 h-full overflow-y-auto">
      <h2 className="text-xl text-white font-bold mb-2">Smart Recommendations</h2>
      <p className="text-slate-400 text-sm mb-6">Based on your current stock</p>
      
      <div className="space-y-4">
        {recipes.map(recipe => {
          const match = getMatchStatus(recipe.ingredients);
          
          return (
            <div key={recipe.id} className="bg-slate-800 rounded-2xl p-4 border border-slate-700 flex space-x-4">
                 ...
                 <div className="mt-2 mb-3">
                   <div className="flex items-center space-x-2 text-xs mb-1">
                      ...
                      <div className="flex-1 h-1.5 bg-slate-700 rounded-full max-w-[100px]">
                        <div className="h-full bg-green-500 rounded-full" style={{ width: `${match.percentage}%` }}></div>
                      </div>
                      <span className={match.percentage === 100 ? 'text-green-400' : 'text-yellow-400'}>{match.percentage}%</span>
                   </div>
                   <div className="text-xs text-slate-500">
                      Missing: {match.missing.length > 0 ? match.missing.join(', ') : 'None'}
                   </div>
                 </div>
